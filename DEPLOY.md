...
- **Node 22** — версия зафиксирована в `.nvmrc` и `package.json` (`engines`). Драйвер базы
  `better-sqlite3` собирается под конкретную мажорную версию Node, и на другой падает при
  ПЕРВОМ запросе, а не при сборке: `npm run build` проходит, `/api/health` отвечает
  `503 database_unreachable`, и причина в логе выглядит как проблема с базой. Симптом при
  прямом обращении — `NODE_MODULE_VERSION 127 … requires 115`. Лечение: `nvm use`, либо
  `npm rebuild better-sqlite3` той версией Node, которой будете запускать.
...
## Вариант Б: hostd + Cloudflare Tunnel

Готовый комплект для машины, где :80/:443 уже заняты чужим веб-сервером (aaPanel,
чужие сайты) и трогать его нельзя. Публичных портов у CRM в этом варианте ноль:
наружу смотрит Cloudflare Tunnel — контейнер `cloudflared` держит исходящее
соединение к Cloudflare и передаёт запросы на `http://crm:3000` по приватной
docker-сети. TLS терминируется на границе Cloudflare, Caddy и шаги 6–7 выше не нужны.

```
  интернет ──▶ Cloudflare (crm.agintent.com) ──▶ туннель ──▶ crm:3000
  оператор ──▶ ssh -p 222 ──▶ curl 127.0.0.1:3080/api/health
```

Всё лежит в `deploy/hostd/` и описано в его `README.md`:

- `docker-compose.prod.yml` — те же образ, том и healthcheck, что выше, плюс сервис
  `cloudflared`; приложение опубликовано только на `127.0.0.1:3080` для оператора;
- `.env.production.example` — значения под эту схему с комментариями;
- `deploy.sh` — деплой с рабочей станции одним запуском: rsync → build → up →
  проверка `/api/health` через loopback машины;
- `backup-cron.sh` и `healthwatch.sh` — ночной снимок с копией на диск хоста и
  пятиминутный сторож с одним рестартом при отказе.

Два места, где эта схема отличается от основной, и оба стоят внимания:

- **`TRUSTED_PROXY_HOPS` остаётся `1`.** Таблица выше говорит «CDN перед прокси делает
  значение двойкой» — это про цепочку CDN → Caddy, где к `X-Forwarded-For` дописывают
  двое. Здесь дописывает только граница Cloudflare: `cloudflared` передаёт заголовки
  без собственной приписки, локального прокси нет вовсе. Правый край заголовка — адрес
  клиента, единица читает именно его; двойка сдвинулась бы влево, в текст, который
  прислал сам клиент, и открыла бы обход тротлинга входа.
- **Wildcard живёт в туннеле.** Public hostname `*.crm.agintent.com → http://crm:3000`
  в панели Zero Trust плюс CNAME `*.crm` на `<tunnel-id>.cfargotunnel.com` — иначе
  поддомены контор снаружи никуда не ведут. `CRM_HOME_HOST=crm.agintent.com` при этом
  говорит приложению, что этот хост — парадный вход, а слаги начинаются уровнем ниже.

---

## Домашний адрес продукта: `CRM_HOME_HOST`

Приложение читает рабочее пространство из первой метки адреса: `acme.agintent.com` —
это контора «acme». По этому же правилу собственный адрес продукта вида
`crm.agintent.com` читался бы как контора «crm», и вход с регистрацией остались бы
без своего поддомена.

`CRM_HOME_HOST` называет этот домашний адрес:

```bash
CRM_HOME_HOST=crm.agintent.com
```

С этим значением запрос на `crm.agintent.com` ведёт себя ровно как голый апекс
сегодня (пространство `demo`, вход и регистрация), а соседние поддомены —
`acme.agintent.com`, `beta.agintent.com` — по-прежнему читаются как конторы.
Сравнение без учёта регистра, порт в адресе игнорируется. Пустая или отсутствующая
переменная оставляет поведение прежним по всем адресам.

Переменная серверная: middleware и API видят её сразу, клиентский бандл — нет.
Ссылки на страницу входа с домашнего адреса должны называть контору явно, через
`?slug=<контора>`.
